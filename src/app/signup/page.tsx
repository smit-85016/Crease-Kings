...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupFormValues) => {
    ...
    // Simulate signup success/failure (e.g., check if email exists)
    // In a real app, you'd send data to your backend for registration
    if (data.email === 'existing@creasekings.fake') {
      toast({
        title: 'Signup Failed',
    ...
    <main className="flex min-h-screen items-center justify-center bg-secondary p-4">
      <Card className="w-full max-w-md shadow-xl">
        <CardHeader className="text-center space-y-2">
          ...
          <CardDescription>Join Crease Kings and start booking!</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-1"><User className="h-4 w-4 text-muted-foreground"/>Full Name</FormLabel>
                    <FormControl>
